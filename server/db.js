...
export function createUser(email, passwordHash, accessType = "per_story", isAdmin = false) {
  const id = randomUUID();
...
export function addStoryAccess(userId, storyId) {
  const stmt = db.prepare(
...
export function hasStoryAccess(userId, storyId) {
...
export function getUserPurchasedStories(userId) {
  const stmt = db.prepare(
...
export function setUserAdmin(userId, isAdmin = true) {
  const stmt = db.prepare("UPDATE users SET is_admin = ? WHERE id = ?");
  stmt.run(isAdmin ? 1 : 0, userId);
}

export function setAdminByEmail(email) {
...
export function updatePasswordByEmail(email, passwordHash) {
...
export function seedStoriesIfEmpty() {
  if (countStories() > 0) return;
  const defaults = [
  ...
  defaults.forEach((d) => createStory(d));
}

...
const dbPath = path.join(dataDir, "story-sanctuary.db");
...
function createUserSync(email, passwordHash, accessType = "per_story", isAdmin = false) {
  ...
  const stmt = db.prepare(
    "INSERT INTO users (id, email, password_hash, access_type, is_admin) VALUES (?, ?, ?, ?, ?)"
  );
  stmt.run(id, email.toLowerCase().trim(), passwordHash, accessType, isAdmin ? 1 : 0);
  return { id, email: email.toLowerCase().trim(), access_type: accessType, is_admin: isAdmin };
}

function findUserByEmailSync(email) {
  const stmt = db.prepare(
    "SELECT id, email, password_hash, access_type, is_admin, created_at FROM users WHERE email = ?"
  );
...
function findUserByIdSync(id) {
  const stmt = db.prepare(
    "SELECT id, email, access_type, is_admin, created_at FROM users WHERE id = ?"
  );
...
function addStoryAccessSync(userId, storyId) {
  const stmt = db.prepare(
    "INSERT OR IGNORE INTO user_stories (user_id, story_id) VALUES (?, ?)"
  );
  stmt.run(userId, storyId);
}

function hasStoryAccessSync(userId, storyId) {
  ...
  return stmt.get(userId, storyId) != null;
}

function getUserPurchasedStoriesSync(userId) {
  const stmt = db.prepare(
    "SELECT story_id FROM user_stories WHERE user_id = ?"
  );
...
function listStoriesSync() {
  const stmt = db.prepare(
    "SELECT * FROM stories ORDER BY published_at DESC, created_at DESC"
  );
  return stmt.all().map(rowToStory);
}

function getStoryByIdSync(id) {
  const stmt = db.prepare("SELECT * FROM stories WHERE id = ?");
  return rowToStory(stmt.get(id));
}

function createStorySync(data) {
  ...
  return getStoryByIdSync(id);
}

function updateStorySync(id, data) {
  const existing = getStoryByIdSync(id);
...
function deleteStorySync(id) {
  const stmt = db.prepare("DELETE FROM stories WHERE id = ?");
  stmt.run(id);
  return true;
}

function setUserAdminSync(userId, isAdmin = true) {
  const stmt = db.prepare("UPDATE users SET is_admin = ? WHERE id = ?");
  stmt.run(isAdmin ? 1 : 0, userId);
}

function setAdminByEmailSync(email) {
  const user = findUserByEmailSync(email);
  if (!user) return false;
  setUserAdminSync(user.id, true);
  return true;
}

function updatePasswordByEmailSync(email, passwordHash) {
  const stmt = db.prepare("UPDATE users SET password_hash = ? WHERE email = ?");
  stmt.run(passwordHash, email.toLowerCase().trim());
  return stmt.changes > 0;
}

function countStoriesSync() {
  const stmt = db.prepare("SELECT COUNT(*) as n FROM stories");
  return stmt.get().n;
}
...
export const addStoryAccess = (...a) => Promise.resolve(addStoryAccessSync(...a));
export const hasStoryAccess = (...a) => Promise.resolve(hasStoryAccessSync(...a));
export const getUserPurchasedStories = (...a) => Promise.resolve(getUserPurchasedStoriesSync(...a));
export const listStories = (...a) => Promise.resolve(listStoriesSync(...a));
export const getStoryById = (...a) => Promise.resolve(getStoryByIdSync(...a));
export const createStory = (...a) => Promise.resolve(createStorySync(...a));
export const updateStory = (...a) => Promise.resolve(updateStorySync(...a));
export const deleteStory = (...a) => Promise.resolve(deleteStorySync(...a));
export const setUserAdmin = (...a) => Promise.resolve(setUserAdminSync(...a));
export const setAdminByEmail = (...a) => Promise.resolve(setAdminByEmailSync(...a));
export const updatePasswordByEmail = (...a) => Promise.resolve(updatePasswordByEmailSync(...a));
export const countStories = (...a) => Promise.resolve(countStoriesSync(...a));
export const seedStoriesIfEmpty = (...a) => Promise.resolve(seedStoriesIfEmptySync(...a));
